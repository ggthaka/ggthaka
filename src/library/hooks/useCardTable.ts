...
export default function useCardTable<T extends Record<string, unknown>>(
  endpoint: string,
  columns: ColumnDef<T>[],
  features: Features = {},
  ...
  useEffect(() => {
    ...
    window.addEventListener('cardtable:refresh', onRefresh);
    return () => window.removeEventListener('cardtable:refresh', onRefresh);
  }, []);
  ...
  const handleDragStart = useCallback(
    (key: string) => (event: DragEvent<HTMLElement>) => {
  ...
  const startResize = useCallback(
    (key: string) => (event: ReactMouseEvent<HTMLElement>) => {
      ...
      const resizableHost = target.closest(
        '[data-cardtable-resize-host]',
      ) as HTMLElement | null;
      const width =
        resizableHost?.getBoundingClientRect().width ??
